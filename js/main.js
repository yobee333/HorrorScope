...
let dayInput = Number(document.querySelector('#date').value) //add Number to number
    ...
    function postResetButton(){
        const resetButtonContainer = document.getElementById('#reset')
        resetButtonContainer.innerHTML = '';

        const resetButton = `
        <h2>Reset to check on your friends</h2>
        <input class="button" 
        type="button" 
        onclick="location.href='javascript:confirmRefresh()'" 
        value="Reset" 
        />
        `

        resetButtonContainer.insertAdjacentHTML("beforeend", resetButton);
      }
    }
